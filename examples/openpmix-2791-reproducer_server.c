...
static pmix_status_t server_client_connected2_cb(const pmix_proc_t *proc,
						 ...
						 void *cbdata)
{
    printf("Client connected: rank %d, nspace '%s')\n",
...
{
    printf("%s(nspace %s rank %d srvtmpdir %s systmpdir %s)\n",
	...
	printf("%s: filling server session info failed\n", __func__);
	return false;
    }
    ...
    printf("%s: PMIx_server_register_resources() successful\n", __func__);
				     ...
				     const char *tmpdir, const char *nsdir)
{
    ...
    uint32_t univSize = jobSize;
    PMIX_INFO_LOAD(&data.info[i], PMIX_UNIV_SIZE, &jobSize, PMIX_UINT32);
    i++;

    /* session info array */
    pmix_data_array_t sessionInfo;
    fillSessionInfoArray(&sessionInfo, 12345, univSize);
    ...
    printf("%s: PMIx_server_register_nspace info:\n", __func__);
...
bool pspmix_server_setupLocalSupport(const char *nspace)
{
    ...
    if (status != PMIX_SUCCESS) {
	printf("%s: Setting up the local support callback failed: %s\n",
    ...
    if (cbdata.status != PMIX_SUCCESS) {
	printf("%s: Callback from setup local support failed: %s\n", __func__,
	     PMIx_Error_string(cbdata.status));
	return false;
    }
    DESTROY_CBDATA(cbdata);
...
{
    printf("%s(nspace '%s' rank %d uid %d gid %d)\n", __func__,
	 nspace, rank, uid, gid);
    ...
    if (status != PMIX_SUCCESS) {
	printf("%s: Registering client failed: %s\n", __func__,
	     PMIx_Error_string(status));
	return false;
    }
    WAIT_FOR_CBDATA(cbdata);
    ...
    if (cbdata.status != PMIX_SUCCESS) {
	printf("%s: Callback from register client failed: %s\n", __func__,
	     PMIx_Error_string(cbdata.status));
	return false;
    }
    DESTROY_CBDATA(cbdata);
...
bool pspmix_server_setupFork(const char *nspace, int rank, char ***childEnv)
{
    ...
    PMIX_PROC_DESTRUCT(&proc);
    if (status != PMIX_SUCCESS) {
	printf("%s: Setting up environment for fork failed: %s\n",
	     __func__, PMIx_Error_string(status));
	return false;
    }
...
bool pspmix_server_finalize(void)
{
    ...
    if (status != PMIX_SUCCESS) {
	printf("%s: PMIx_server_finalize() failed: %s\n", __func__,
	     PMIx_Error_string(status));
	return false;
    }
    return true;
...
int main(void) {
    ...
    snprintf(nspace, PMIX_MAX_NSLEN, "pspmix_nspace");
    ...
    /* register namespace */
    if (!pspmix_server_registerNamespace(nspace, jobSize,
					 "/tmp/pmix-session-12345",
					 "/tmp/pmix-session-12345/nspace")) {
    ...
    for (size_t rank = 0; rank < jobSize; rank++) {

	if (!pspmix_server_registerClient(nspace, rank, getuid(), getgid(),
					  &rank)) {
	    printf("%s: Registering client rank %zu failed\n", __func__, rank);
	    return false;
	}

	char **envp = calloc(sizeof(*envp), 1);
	if (!pspmix_server_setupFork(nspace, rank, &envp)) {
	    printf("%s: Setting up environment for rank %zu failed\n", __func__,
		   rank);
	    free(envp);
	    pspmix_server_deregisterClient(nspace, rank);
	    return false;
	}

	printf("Environment for rank %zu:\n", rank);
	for (size_t i = 0; envp[i]; i++) {
	    char *val = strstr(envp[i], "=");
	    *(val++) = '\0';
	    printf("export %s='%s'\n", envp[i], val);
	}
	free(envp);
